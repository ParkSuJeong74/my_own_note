...
## 서버, Docker, PostgreSQL 모니터링

Main Org.에는 다음 관리자 전용 대시보드가 자동으로 provision됩니다.

- `Server Overview`: 서버 CPU, 메모리, 루트 디스크, load average, 네트워크
- `Docker Overview`: 컨테이너별 CPU, 메모리, 네트워크 사용량
- `PostgreSQL Overview`: Alcove/Tono DB 상태, 연결 수, 크기, lock, transaction
...
cd ~/alcove_be
doppler run -- docker compose up -d postgres-exporter
