...
import { githubRepository, rerunWorkflow } from "@/lib/github-actions";
...
export async function rerunGitHubWorkflowAction(data:FormData){const workspaceId=value(data,"workspaceId"),slug=value(data,"slug"),owner=value(data,"owner"),repo=value(data,"repo"),runId=Number(value(data,"runId"));if(!workspaceId||!slug||!owner||!repo||!Number.isSafeInteger(runId)||runId<=0)return;const workspace=(await listWorkspaces()).find((item)=>item.id===workspaceId&&item.slug===slug);const repository=workspace?.links.map((link)=>githubRepository(link.url)).find((item)=>item?.owner.toLowerCase()===owner.toLowerCase()&&item.repo.toLowerCase()===repo.toLowerCase());if(!repository)return;await rerunWorkflow(repository,runId);refresh(slug);}
