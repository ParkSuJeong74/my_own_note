...
export async function deleteEventAction(data:FormData){const id=text(data,"id");if(!id)return;await deleteCalendarEvent(id);revalidatePath("/calendar");revalidatePath("/");redirect(calendarReturn(data));}
