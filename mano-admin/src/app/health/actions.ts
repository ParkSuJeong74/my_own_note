...
const measurement=(data:FormData)=>({measuredOn:date(data,"measuredOn")??"",weightKg:positive(data,"weightKg",true) as number,bodyFatPct:percentage(data,"bodyFatPct"),skeletalMuscleKg:positive(data,"skeletalMuscleKg"),waistCm:positive(data,"waistCm"),note:value(data,"note")});
