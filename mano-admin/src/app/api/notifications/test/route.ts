...
const destination = (request: Request, result: string) => NextResponse.redirect(new URL(`/notifications?test=${result}`, request.url), 303);
export async function POST(request: Request) {
  if (!request.headers.get("x-mano-admin-user")) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!ntfyConfigured()) return destination(request, "not-configured");
  try {
    await sendNtfyNotification({ title: "Mano 알림 테스트", message: "ntfy 알림이 정상적으로 연결되었습니다.", tags: ["white_check_mark"] });
    return destination(request, "sent");
  } catch (error) {
