...
export async function POST(request:Request){
  ...
  const body=await request.json().catch(()=>null) as {comments?:CollectedBlogReply[]}|null;
  if(!Array.isArray(body?.comments))return NextResponse.json({error:"comments_required"},{status:400,headers:cors});
  return NextResponse.json(await ingestBlogReplies(body.comments),{headers:cors});
}
