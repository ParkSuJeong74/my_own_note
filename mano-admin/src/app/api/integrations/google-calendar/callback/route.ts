...
import { connectGoogleCalendar, pullGoogleCalendarChanges, retryGoogleCalendarSync } from "@/lib/google-calendar";
...
export async function GET(request: Request) {
  ...
  try { await connectGoogleCalendar(code, new URL("/api/integrations/google-calendar/callback", request.url).toString()); await retryGoogleCalendarSync(); await pullGoogleCalendarChanges(true); target.searchParams.set("google", "connected"); }
