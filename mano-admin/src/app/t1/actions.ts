...
import {createT1Match,deleteT1Game,deleteT1Match,syncT1FromLeaguepedia,syncT1GameDetails,updateFinishedT1Pom,updateT1Match,upsertT1Game} from "@/lib/t1-repository";
...
export async function updateT1PomAction(data:FormData){const id=value(data,"id"),pomPlayer=value(data,"pomPlayer");if(id&&pomPlayer)await updateFinishedT1Pom(id,pomPlayer);refresh();}
export async function syncT1GameDetailsAction(data:FormData){const matchId=value(data,"matchId"),gameNumber=number(data,"gameNumber");if(!matchId||gameNumber<1)return;try{const result=await syncT1GameDetails(matchId,gameNumber);if(!result.updated)await recordAdminError("t1-game-detail-sync",new Error(result.skipped?`세트 상세 동기화 건너뜀: ${result.skipped}`:"공식 제공처에 해당 세트 상세 데이터가 아직 없습니다."),{matchId,gameNumber,...result});}catch(error){await recordAdminError("t1-game-detail-sync",error,error instanceof ExternalProviderError?{provider:error.provider,operation:error.operation,httpStatus:error.status,attempts:error.attempts,matchId,gameNumber}:{matchId,gameNumber});}refresh();}
