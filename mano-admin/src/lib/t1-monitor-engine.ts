...
export type T1MonitorEvent = { matchId: string; eventType: T1MonitorEventType; team1: "T1"; team2: string; score1: number; score2: number; scheduledAt: string; updatedAt: string };
...
export async function evaluateT1MatchMonitor(match: T1MonitorMatch, current: T1MonitorState, dependencies: MonitorDependencies) {
  ...
  if (policy.shouldFetch || finalConfirmationDue) {
    ...
    if (snapshot) {
      ...
      if (scoreChanged && await dependencies.notify(event("T1_SET_RESULT_CHANGED", snapshot.t1Score, snapshot.opponentScore), notificationMatch)) { state.lastKnownT1Score = snapshot.t1Score; state.lastKnownOpponentScore = snapshot.opponentScore; state.lastKnownSetNumber = snapshot.t1Score + snapshot.opponentScore; notificationsCreated++; }
