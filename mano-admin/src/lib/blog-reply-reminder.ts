...
export async function sendOverdueBlogReplyReminder() {
  ...
  try {
    ...
    const due=await client.query(`SELECT commenter,post_url,comment_excerpt,commented_at FROM blog_reply_items WHERE workspace_id=$1 AND replied_at IS NULL AND commented_at<=now()-interval '72 hours' ORDER BY commented_at`,[workspaceId]),unique=[...new Map(due.rows.map(row=>[blogReplySourceKey({postUrl:row.post_url,commenter:row.commenter,commentExcerpt:row.comment_excerpt,commentedAt:new Date(row.commented_at).toISOString()}),row])).values()].slice(0,20);
