...
export async function ingestBlogReplies(items: CollectedBlogReply[], repliedItems: CollectedBlogReply[] = [], reopenPending = false) {
  ...
  try{
    ...
    const existing=await client.query(`SELECT id,post_url,commenter,comment_excerpt,commented_at,replied_at,reply_status_source FROM blog_reply_items WHERE workspace_id=$1 ORDER BY created_at,id FOR UPDATE`,[workspaceId]),normalizedRows=existing.rows.map(row=>({id:String(row.id),postUrl:String(row.post_url),commenter:String(row.commenter),commentExcerpt:String(row.comment_excerpt),commentedAt:new Date(row.commented_at).toISOString(),repliedAt:row.replied_at?new Date(row.replied_at):null,replyStatusSource:row.reply_status_source?String(row.reply_status_source):null})),legacyRows=normalizedRows.filter(row=>!hasNaverBlogPostIdentity(row.postUrl));
    ...
    for(const [key,group] of groups){const [keeper,...duplicates]=group,repliedAt=earliestBlogReplyDate(group.map(item=>item.repliedAt)),replyStatusSource=group.some(item=>item.replyStatusSource==="MANUAL")?"MANUAL":group.some(item=>item.replyStatusSource==="COLLECTED")?"COLLECTED":null;if(duplicates.length){await client.query(`DELETE FROM blog_reply_items WHERE workspace_id=$1 AND id=ANY($2::uuid[])`,[workspaceId,duplicates.map(item=>item.id)]);cleaned+=duplicates.length;}await client.query(`UPDATE blog_reply_items SET source_key=$3,replied_at=COALESCE(replied_at,$4),reply_status_source=$5,updated_at=CASE WHEN $4::timestamptz IS NOT NULL AND replied_at IS NULL THEN now() ELSE updated_at END WHERE workspace_id=$1 AND id=$2`,[workspaceId,keeper.id,key,repliedAt,replyStatusSource]);identityMap.set(key,[keeper.id]);}
    let accepted = 0;
    for (const item of items.slice(0, 100)) {
      ...
      const key=blogReplySourceKey({...item,commenter,commentExcerpt:excerpt}),existingIds=identityMap.get(key);if(existingIds?.[0]){await client.query(`UPDATE blog_reply_items SET post_url=$3,commenter=$4,comment_excerpt=$5,replied_at=CASE WHEN $6 AND reply_status_source IS DISTINCT FROM 'MANUAL' THEN NULL ELSE replied_at END,reply_status_source=CASE WHEN $6 AND reply_status_source IS DISTINCT FROM 'MANUAL' THEN NULL ELSE reply_status_source END,updated_at=now() WHERE workspace_id=$1 AND id=$2`,[workspaceId,existingIds[0],item.postUrl.trim(),commenter,excerpt,reopenPending]);continue;}
    ...
    const repliedIds=[...new Set(repliedKeys.flatMap(key=>identityMap.get(key)??[]))],completed=repliedIds.length?(await client.query(`UPDATE blog_reply_items SET replied_at=COALESCE(replied_at,now()),reply_status_source=CASE WHEN reply_status_source='MANUAL' THEN 'MANUAL' ELSE 'COLLECTED' END,updated_at=now() WHERE workspace_id=$1 AND id=ANY($2::uuid[]) AND (replied_at IS NULL OR reply_status_source IS DISTINCT FROM 'MANUAL')`,[workspaceId,repliedIds])).rowCount??0:0;
    await client.query("COMMIT");
