...
export type MoneyAccount = { id:string; name:string; accountType:"CASH"|"BANK"|"INVESTMENT"|"DEBT"; status:"ACTIVE"|"ENDED"; bankName:string; balance:number; monthlyAmount:number; interestRate:number; note:string; maturityDate:string|null; updatedAt:string };
export type MoneyFixedExpense = { id:string; name:string; amount:number; paymentDay:number|null; note:string; updatedAt:string };
