...
블로그 수집 확장은 네이버 관리 화면의 iframe을 각각 검사합니다. 이웃 목록은 숫자
페이지 버튼이 내부 스크립트 방식이어도 다음 페이지의 목록이 실제로 바뀔 때까지
순회하며, 댓글 관리는 메뉴 프레임과 댓글 행 프레임이 달라도 행을 합칩니다. 성장
스냅샷은 선택 날짜의 값만 합치고 통계 화면에서 제공하지 않는 방문자·게시글을 다른
날짜 값으로 채우지 않습니다. 실제로 관측되지 않은 지표는 Admin에서 `—`로 표시합니다.
