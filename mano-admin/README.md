...
달라도 행을 합치고, 주소로 접근 가능한 숫자 페이지를 모두 순회합니다. 댓글 관리 목록은
댓글과 내 답글의 실제 부모 관계를 제공하지 않으므로 같은 글의 작성 시각만으로 답글 완료를
추론하지 않습니다. 주소 없는 페이지 버튼은 자동 이동하지 않고 부분 수집임을 결과에 표시합니다. 성장
...
댓글 관리의 JavaScript 페이지네이션은 숨은 page 입력과 `currentPage`, `page`, `pageNo`
후보 중 실제 새 댓글을 반환하는 파라미터를 선택해 최대 100페이지까지 순회합니다.
글 페이지에서는 네이버 댓글의 실제 스레드 안에 내 답글이 있는 경우에만 완료 처리합니다.
완료 상태에는 `MANUAL`과 `COLLECTED` 출처를 기록합니다. 글 페이지에서 미답글로 정확히
재수집되면 과거 자동 완료와 출처가 없던 레거시 완료는 복구하지만 Admin에서 누른 수동
완료는 유지합니다. 관리 목록 수집은 기존 완료 상태를 다시 열지 않습니다.
댓글 관리 전체 수집은 목록에서 발견한 게시글 `PostView`로 숫자 블로그 번호를 구한 뒤
현재 네이버 관리 탭에서 로그인된 CBOX JSONP 댓글 API를 읽습니다. 작성자와 작성 분이 같은 실제 최상위 댓글의
하위 답글에 `mine`/관리자 표시가 있는지 대조한 뒤 미답글과 완료 댓글을 전송합니다.
정적 게시글 응답에 댓글 DOM이 없는 경우도 지원하며, 게시글 또는 댓글 API를 읽지 못했거나
실제 댓글과 대응하지 않는 관리 행은 추측해 미답글로 넣지 않고 검증 불가 건수로 표시합니다.
...
- 기본 일정 동기화는 Leaguepedia의 경기 ID를 기준으로 경기 정보를 갱신한다.
- 가까운 예정 경기의 상대가 `TBD` 또는 `TBA`로 남아 있으면 공식 LoL Esports LCK
  일정에서 같은 시각대의 T1 경기를 찾아 확정 상대만 보정한다.
- 공식 일정에서도 상대가 미정이거나 같은 시각대의 T1 경기를 하나로 특정하지 못하면 기존
  값을 유지한다. 이미 확정된 상대를 보조 데이터로 덮어쓰지 않는다.
- 검증은 공식 일정 HTML 파싱, 시간 허용 범위, 미정 상대 제외와 확정 상대 보정을 포함한다.
