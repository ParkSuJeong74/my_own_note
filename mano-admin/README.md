...
댓글 관리 전체 수집도 목록에서 발견한 게시글을 실제 본문 iframe과 같은 `PostView`
렌더링 파라미터로 추가로 읽고, 작성자와
작성 분이 같은 실제 최상위 댓글의 답글 영역을 대조한 뒤 미답글과 완료 댓글을 전송합니다.
게시글을 읽지 못했거나 실제 댓글과 대응하지 않는 관리 행은 추측해 미답글로 넣지 않고
검증 불가 건수로 표시합니다.
